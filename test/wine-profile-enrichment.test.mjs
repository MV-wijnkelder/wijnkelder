...
test("an empty profile is generated, persisted, and returned", async () => {
  ...
  const result = await enrichWineProfile(wine, { async generateWineProfile() { generated++; return profile; } }, { async updateProfile(id, value) { persisted++; assert.equal(id, 7); assert.deepEqual(value, profile); return { ...wine, profile: value }; } });
...
test("a temporary enrichment failure leaves the wine usable and retryable", async () => {
  let saved = false;
  const result = await enrichWineProfile(wine, { async generateWineProfile() { throw new Error("temporary outage"); } }, { async updateProfile() { saved = true; return null; } });
