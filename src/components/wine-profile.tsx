...
export function WineProfile({ wine, bottleCount }: { wine: Wine; bottleCount?: number }) {
  ...
  const maturityAdvice = profile.drinking.currentMaturity
    ...
    : null;
  const quantity = bottleCount ?? 0;
  const marketValue = wine.marketValue === null ? null : money(wine.marketValue, wine.marketValueCurrency ?? "EUR");
  const totalMarketValue = wine.marketValue === null ? null : money(wine.marketValue * quantity, wine.marketValueCurrency ?? "EUR");
  return <article className="wine-profile">
    ...
    {bottleCount !== undefined && <ProfileSection title="Market valuation"><Details values={[["Current Market Value (per bottle)", marketValue ?? "Not available yet"], ["Quantity", `${quantity} ${quantity === 1 ? "bottle" : "bottles"}`], ["Total Market Value", totalMarketValue ?? "Not available yet"], ["Status", marketValue === null ? "Awaiting market valuation" : "Current estimate"]]} /></ProfileSection>}
