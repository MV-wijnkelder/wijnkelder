...
export function WineProfile({ wine, bottleCount }: { wine: Wine; bottleCount?: number }) {
  ...
  return <article className="wine-profile">
    ...
    {bottleCount !== undefined && <ProfileSection title="Estimated Market Value"><Details values={[["Estimated Market Value", marketValue ?? "Currently unavailable"], ["Evidence", wine.marketValueMetadata.observationCount > 0 ? `Based on ${wine.marketValueMetadata.observationCount} current public ${wine.marketValueMetadata.observationCount === 1 ? "offer" : "offers"}` : null], ["Status", marketValue && wine.marketValueMetadata.lastAttemptFailure ? `Previous estimate — last verified ${formatDate(wine.marketValueMetadata.retrievedAt)}` : null]]} /></ProfileSection>}
...
function money(value: number, currency: string): string { return new Intl.NumberFormat("en-IE", { style: "currency", currency }).format(value); }
