...
import { PremiumButton, PremiumLink } from "@/components/premium-ui";
...
const textFields: Array<{
  ...
  { key: "wineColor", label: "Wine color", placeholder: "For example, red" },
  {
    key: "bottleSize",
    label: "Bottle size",
    placeholder: "For example, 750 ml",
  },
];

export function WineReview({
  wine,
  onChange,
  onScanAgain,
  onSave,
  ...
  async function exploreWine() {
    ...
    setSaveError(null);
    try {
      await onExplore(wine);
    } catch (error) {
      setSaveError(
        error instanceof Error
          ? error.message
          : "The wine profile could not be created.",
      );
    } finally {
  ...
  function updateAlcohol(event: ChangeEvent<HTMLInputElement>) {
    ...
    onChange({
      ...wine,
      alcoholPercentage: value === "" ? null : Number(value),
    });
    setSaveMessage(null);
  }
  ...
  async function confirmReview(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSaveMessage(
        result.duplicate
          ? "Wine successfully added. Bottle quantity increased."
          : "Wine successfully added.",
      );
    } catch (error) {
      setSaveError(
        error instanceof Error ? error.message : "The wine could not be saved.",
      );
    } finally {
      setIsSaving(false);
    }
    ...
    >
      <div className="result-heading">
        ...
        <div
          ...
          aria-label={`${wine.confidence}% confidence`}
        >
      ...
      <div className="ai-summary">
        ...
        <div>
          <strong>Recognition complete</strong>
          <p>AI filled in this information. Review and edit it where needed.</p>
        </div>
      </div>
      ...
      {saveMessage ? (
        <div className="review-actions">
          <PremiumButton className="w-full" type="button" onClick={onScanAgain}>
            <ArrowClockwiseIcon className="size-5" />
            Scan another wine
          </PremiumButton>
          <PremiumLink className="w-full" variant="secondary" href="/cellar">
            My cellar
          </PremiumLink>
        </div>
      ) : (
        <div className="review-actions review-actions-three">
          <PremiumButton variant="quiet" type="button" onClick={onScanAgain}>
            Start over
          </PremiumButton>
          <PremiumButton
            className="w-full"
            type="submit"
            disabled={isSaving || isExploring}
          >
            {isSaving ? (
