...
import { drinkingOutlookLabel, drinkingStageLabel, type DrinkingStage } from "@/lib/drinking-lifecycle";
...
function InsightSelection() {
  ...
  return <main className="app-shell premium-page relative min-h-screen overflow-x-clip px-5 py-6 sm:px-6 sm:py-10"><HeroBackground atmosphere="cellar" /><section className="page-enter relative z-10 mx-auto w-full max-w-3xl">
    ...
    {error ? <p className="error-message" role="alert">{error}</p> : loading ? <p className="cellar-status">Loading wines…</p> : selected.length ? <><PremiumButton className="export-action" variant="secondary" onClick={() => downloadCellarWorkbook(selected)}>Export to Excel</PremiumButton><p className="cellar-selection-count">{selected.length} {selected.length === 1 ? "wine" : "wines"}</p><CellarWineList wines={selected} returnTo={returnTo} /></> : <div className="cellar-empty"><WineglassIcon /><strong>No wines found in this selection.</strong></div>}
  </section></main>;
}
function outlookTitle(key: OutlookKey) { return drinkingOutlookLabel(key); }
function readinessTitle(position: number) { const stage = (["", "wellBeyondPeak", "beyondPeak", "pastPeak", "young", "ready", "excellent", "nearPeak", "peak"] as const)[position]; return stage ? drinkingStageLabel(stage as DrinkingStage) : "Drink Readiness"; }
