...
export function hasWineProfile(profile: WineProfile): boolean {
  return Boolean(profile.summary || profile.foodPairings.length ||
    profile.sommelier.occasions.length || Object.values(profile.sommelier.pairings).some((items) => items.length) ||
    profile.sommelier.wineStyle || profile.sommelier.ageingPotential || profile.sommelier.drinkingStage || profile.sommelier.servingPersonality ||
